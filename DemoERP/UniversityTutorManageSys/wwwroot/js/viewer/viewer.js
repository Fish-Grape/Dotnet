...
function (i) {
    ...
    u.prototype = {
        ...
        init: function () {
            ...
            })) : jQuery(document).off(_, e).on(_, e, i.proxy(this.start, this))))
            //  this.build()
        },
        ready: function () {
        ...
        build: function () {
			...
			h.inline ? (a.addClass(P), e.css("z-index", h.zIndexInline),
			"static" === t.css("position") && t.css("position", "relative")) : (a.addClass(V), e.css("z-index", h.zIndex).addClass([w, z, y].join(" "))),
			t.append(e), h.inline && (this.render(), this.bind(), this.isShown = !0), this.isBuilt = !0, i.isFunction(h.built) && r.one(B, h.built), this.trigger(B))
        },
        unbuild: function () {
            var i = this.options,
				t = this.$element;
        ...
        bind: function () {
			...
			/*jQuery(document).off(_, this.$viewer).on(_, this.$viewer, i.proxy(this.click, this)),
            jQuery(document).off(_, this.$viewer).on(q, this.$viewer, i.proxy(this.wheel, this)),
			jQuery(document).off(S, this.$canvas).on(S, this.$canvas, i.proxy(this.mousedown, this)),
			jQuery(document).off(D, v).on(D, v, this._mousemove = o(this.mousemove, this)),
            jQuery(document).off(L, v).on(L, v, this._mouseup = o(this.mouseup, this)),
            jQuery(document).off(W, v).on(W, v, this._keydown = o(this.keydown, this)),
			jQuery(document).off(j, m).on(j, m, this._resize = o(this.resize, this))*/
			
			
			this.$viewer.off(_, this.$viewer).on(_, this.$viewer, i.proxy(this.click, this)),
            this.$viewer.off(_, this.$viewer).on(q, this.$viewer, i.proxy(this.wheel, this)),
			this.$viewer.off(S, this.$canvas).on(S, this.$canvas, i.proxy(this.mousedown, this)),
			v.off(D, v).on(D, v, this._mousemove = o(this.mousemove, this)),
            this.$viewer.off(L, v).on(L, v, this._mouseup = o(this.mouseup, this)),
           	jQuery(document).off(W, v).on(W, v, this._keydown = o(this.keydown, this)),
			m.off(j, m).on(j, m, this._resize = o(this.resize, this))
        },
        unbind: function () {
           var t = this.options,
				e = this.$element;
        ...
        show: function () {
            ...
            if (this.$images.index(this.target) < 0) {
                return false;
            }
			...
			e.transition ? (this.transitioning = !0, t.addClass(X), h(t[0]), t.one(R, i.proxy(this.shown, this)).addClass(F)) : (t.addClass(F), this.shown())))
        },
        hide: function () {
            var t = this.options,
				e = this.$viewer;
